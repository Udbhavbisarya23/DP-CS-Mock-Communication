...
int main(int argc, char* argv[]) {  
     ...
     curr_path += "/input_dp_mat.txt";
     indata.open(curr_path);
     int rows,columns;
     indata >> rows;
     indata >> columns;
     ...
     // Reading the constant value from the input_dp_constant file for multiplication
     curr_path = std::filesystem::current_path();
     curr_path += "/input_dp_constant.txt";
     indata.open(curr_path);
     int constant;
     indata >> constant;
     ...
     for(int i=0;i<2;i++) {
          ...
          //Sending the constant to the servers (NOT IN SHARE FORM)
          boost::asio::write( socket, boost::asio::buffer(&constant,sizeof(constant)), error ); 
