...
class Matrix {
    private: 
        ...
        bool readOnlyFirstRow;
        struct Shares *cs0_data, *cs1_data;


    public:
        std::vector<float> data;
        Matrix(int row, int col, std::string fileName, std::size_t fraction_bits, bool onlyFirstRow=false) {
            rows = row;
            columns = col;
            num_elements = row*col;
            fractional_bits = fraction_bits;
            readOnlyFirstRow = onlyFirstRow;            
...
int main(int argc, char* argv[]) {  
    ...
    Matrix weightL1 = Matrix(1,784,"/W1.csv",options->fractional_bits,true);
    ...
    Matrix X = Matrix(784,1,"/X.csv",options->fractional_bits);
    X.readMatrixCSV();
    X.printData();
    X.generateShares();
    X.sendToServers();
    ...
    std::cout << "W1 data size:- " << weightL1.data.size() << "\n";

    float ans = 0;
    for(int i=0;i<784;i++) {
        ans += (X.data[i]*weightL1.data[i]);
    }
    std::cout << "Final answer should be :- " << ans << "\n";
